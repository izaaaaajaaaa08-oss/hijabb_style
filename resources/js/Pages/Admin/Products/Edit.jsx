...
export default function Edit({ product, categories = [] }) {
    ...
    const handleSubmit = (e) => {
        e.preventDefault();
        // Since Laravel handles files via multipart/form-data with PUT spoofing
        post(route('admin.products.update', product.id));
    };

    return (
        <AdminLayout title={`Edit ${product.name} — HijabStyle Admin`}>
                            ...
                            <div className="sm:col-span-2">
                                <label className="flex items-center gap-2 cursor-pointer">
                                    ...
                                    <span className="text-xs font-medium text-brand-text">
                                        Aktifkan produk ini di katalog toko
                                    </span>
                                </label>
                            </div>
                        </div>
                    ...
                    <div className="p-6 rounded-3xl bg-white border border-gray-200 shadow-sm space-y-4">
                        <div className="flex items-center justify-between pb-2 border-b border-gray-100">
                            <div>
                                ...
                                <p className="text-[11px] text-gray-500">Sesuaikan stok dan harga untuk setiap pilihan warna.</p>
                            </div>
                            <button
                                type="button"
                    ...
                    <div className="p-6 rounded-3xl bg-white border border-gray-200 shadow-sm space-y-4">
                        ...
                        <div className="space-y-3">
                            {product.images && product.images.length > 0 && (
                                <div>
                                    <span className="block text-xs font-semibold text-brand-text mb-2">Foto Saat Ini:</span>
                                    <div className="flex gap-3">
                                        {product.images.map((img) => (
                                            <div key={img.id} className="w-20 h-20 rounded-2xl overflow-hidden border border-gray-200 bg-brand-cream">
                                                <img
                                                    src={`/${img.image_path}`}
                                                    alt=""
